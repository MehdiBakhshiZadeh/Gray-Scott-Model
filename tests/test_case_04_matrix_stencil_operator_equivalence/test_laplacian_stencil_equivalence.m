...
thisDir  = fileparts(thisFile);   % .../tests/test_case_04_matrix_stencil_operator_equivalence
testsDir = fileparts(thisDir);    % .../tests
rootDir  = fileparts(testsDir);   % project root

% Make this test runnable independently
if isempty(which("defaultParams")) || isempty(which("buildStencil2D"))
    addpath(rootDir);
    addpath(genpath(fullfile(rootDir, "src")));
    addpath(genpath(testsDir));
end

% Local output folder for this test (reserved for any future outputs)
